...
render_tool_header(
    ToolInfo(
        title="Kaplan-Meier Curve Maker",
        icon="📉",
        purpose="Create a publication-ready Kaplan-Meier survival or event-free probability curve from a TriNetX export.",
        use_when="You need a clean time-to-event figure with editable labels and robust parsing warnings.",
        primary_input="TriNetX Kaplan-Meier CSV export.",
        outputs="300 DPI PNG curve and parsed data preview.",
    )
)
render_workflow()

st.sidebar.header("Input")nupload = st.file_uploader("Upload TriNetX Kaplan-Meier CSV", type=["csv", "txt"])
st.sidebar.header("Cohort labels")
...
show_ci = st.sidebar.checkbox("Show confidence intervals when detected", value=True)
...
st.subheader("1. Confirm parsed Kaplan-Meier data")
...
if show_ci:
    for surv_col, label in [(surv1, label1), (surv2, label2)]:
        lower = next((c for c in df.columns if surv_col.replace("Survival Probability", "Survival Probability 95 % CI Lower") == c), None)
        upper = next((c for c in df.columns if surv_col.replace("Survival Probability", "Survival Probability 95 % CI Upper") == c), None)
        if lower and upper:
            ax.fill_between(plot_df[time_col], plot_df[lower], plot_df[upper], alpha=0.15, step="post")
